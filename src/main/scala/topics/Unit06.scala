...
*/
  ...
  @main def exercise65(): Unit =
    ...
    val paras: Vector[Picture] = toParagraphs(text) map (flow(20, _).borderLR())
    ...
    val twoColumn: Picture = bulletPoints.grouped(2).toVector.map(_.stack()).spread()
