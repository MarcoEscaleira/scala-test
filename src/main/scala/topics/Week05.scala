...
object Week05 {
  ...
  @main def exercise51(): Unit =
    lazy val a = numList.foldRight(0)(_ + _)
    lazy val b = numList.foldRight(1)(_ * _)
    lazy val c = numList.foldRight(1)(_ max _)
    lazy val d = numList.foldRight(1)(_ min _)
    lazy val e = wordList.foldRight("")(_ + _)
    lazy val f = bitList.foldRight(0)((b, n) => n + 1)
    lazy val g = bitList.foldRight(0)((b, p) => if (b == 1) 1 - p else p)
    lazy val h = bitList.foldRight((0, 0)) { case (b, (z, o)) => (z + 1 - b, o + b) }
    lazy val i = wordList.foldRight("")((w1, w2) => if (w1.length > w2.length) w1 else w2)
    lazy val j = wordList.foldRight(List[String]())((w, ws) => w.reverse :: ws)
    lazy val k = wordList.foldRight(List[String]())((w, ws) => if (w.contains('o')) w :: ws else ws)
    /*println(a)
    println(b)
    println(c)
    println(d)
  ...
  @main def exercise52(): Unit =
    ...
    lazy val b = numList.foldLeft(1)(_ * _)
    lazy val c = numList.foldLeft(1)(_ max _)
    lazy val d = numList.foldLeft(1)(_ min _)
    lazy val e = wordList.foldLeft("")(_ + _) // Here + is string concatenation
    lazy val f = bitList.foldLeft(0)((n, b) => n + 1)
    lazy val g = bitList.foldLeft(0)((p, b) => if (b == 1) 1 - p else p)
    lazy val h = bitList.foldLeft((0, 0)) { case ((z, o), b) => (z + 1 - b, o + b) }
    lazy val i = bitList.foldLeft(0)((d, b) => d * 2 + b)
  ...
  def exercise54(): Unit =
    ...
    //Exercises
    lazy val a = (1 to 9).toList.scanLeft(1)(_ * _)
    lazy val b = "0123".toList.scanRight("")(_ + _)
    lazy val c = "0123".toList.scanLeft("")(_ + _)
    lazy val d = bitList.scanLeft(0)(_ * 2 + _)
    println(s"$a\n$b\n$c\n$d")
    //    println(a)
    //    println(b)
    //    println(c)
  ...
  extension (ys: List[Int])
    def mymap(f: Int => Int): List[Int] = ys.foldRight(List[Int]())((x, xs) => f(x) :: xs)  //use foldRight
    def myfilter(p: Int => Boolean): List[Int] = ys.foldRight(List[Int]())((x, xs) => if p(x) then x :: xs else xs ) //use foldright
    def myreverse: List[Int] = ys.foldLeft(List[Int]())((xs, x) => x :: xs)      // use foldLeft
