...
object Week1 {
  ...
  def subBy(x: Int)(y: Int): Int = ???

  /* smaller returns the smaller of the two arguments */
  def smaller(x: Int)(y: Int): Int = ???
  ...
  @main def exercise16(): Unit =
    ...
    println(nums.map(mul(2) compose add(5)) )
    println(nums.map(add(1).andThen(mul(10))))

    println(nums.map(negate.andThen(add(1))))
    println(nums.map(add(1).compose(negate)))

    println(nums.map(sub(0) andThen add(1) andThen mul(2)))

    println(nums.map((sub(1) compose mul(10) compose add(5))))
    println(nums.map(add(5) andThen mul(10) andThen sub(1)) )
 ...
 // lazy val thrice: (Int => Int) => (Int => Int) = ???
 // lazy val thrice2: (Int => Int) => (Int => Int) = ???
 // lazy val octo: (Int => Int) => (Int => Int) = ???
 // lazy val octo2: (Int => Int) => (Int => Int) = ???
